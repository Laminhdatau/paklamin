...
<?php } else { ?>
    ...
    <script>
        ...
        function editStatus(id_soal, status) {
            $("#id_soal").val(id_soal);
            $("#status").val(status);
            $("#title_addedit").html('<h2>Edit Data : Status</h2>');
            $("#btn").html('Update');
            showForm(id_soal);
        }
    </script>
            ...
            <div class="x_content">
                <div class="row">
                    ...
                    <?php if (($akun[0]->zp[0] == "1") || ($akun[0]->zp[2] == "1")) { ?>
                        <div class="col-sm-12" id="pnladd">
                            <div class="col-sm-12" style="background: #D3D3D3;">
                                <form class="" action="<?php echo base_url() . 'kuismatakuliah/changeStatus'; ?>" method="post" novalidate="">
                                    <!-- spacebar -->
                                    <div style="width: 100%; height:7px; border: 0px solid white;"></div>
                                    <span class="section" id="title_addedit">Status</span>
                                    <div class="field item form-group">
                                        <label class="col-form-label col-md-2 col-sm-2 label-align">Status<span class="required"> *</span></label>
                                        <div class="col-md-8 col-sm-8">
                                        <select name="id_bagian_soal" id="id_bagian_soal" class="form-control">
                                            <?php foreach ($bsoal as $bs) { ?>
                                                <option value="<?= $bs->id_soal; ?>"><?= $bs->status; ?></option>
                                            <?php } ?>
                                        </select></div>
                                    </div>
                                    <div class="ln_solid">
